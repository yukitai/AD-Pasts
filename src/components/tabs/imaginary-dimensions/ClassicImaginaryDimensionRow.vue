...
export default {
  name: "ClassicImaginaryDimensionRow",
  components: {
  ...
  computed: {
    ...
    name() {
      return `${ImaginaryDimension(this.tier).shortDisplayName} Time Dimension`;
    },
    buttonContents() {
  ...
  methods: {
    update() {
      ...
      const dimension = ImaginaryDimension(tier);
    ...
    buyImaginaryDimension() {
      if (!this.isUnlocked) {
        ImaginaryDimension(this.tier).tryUnlock();
        return;
      }
      buySingleImaginaryDimension(this.tier);
    },
    buyMaxImaginaryDimension() {
      buyMaxImaginaryDimension(this.tier);
    },
  }
};
</script>
  ...
  >
    ...
    <div class="l-dim-row-multi-button-container">
      ...
      <PrimaryButton
        ...
        :enabled="isAvailableForPurchase && !isCapped"
        class="o-primary-btn--buy-td-auto"
        @click="buyMaxImaginaryDimension"
      >
        Buy Max
      </PrimaryButton>
    </div>
