...
import ImaginaryDimensionRow from "./ClassicImaginaryDimensionRow";

export default {
  name: "ClassicImaginaryDimensionsTab",
  components: {
    PrimaryButton,
    ImaginaryDimensionRow
  },
  data() {
    return {
      totalUpgrades: 0,
  ...
  computed: {
    costIncreases: () => ImaginaryDimension(1).costIncreaseThresholds,
  },
  methods: {
    update() {
      this.showLockedDimCostNote = !ImaginaryDimension(8).isUnlocked && player.realities >= 1;
      ...
      this.shardsPerSecond.copyFrom(ImaginaryDimension(1).productionPerRealSecond);
    ...
    maxAll() {
      tryUnlockImaginaryDimensions();
      maxAllImaginaryDimensions();
    },
    toggleAllAutobuyers() {
      toggleAllTimeDims();
    }
